...
import { Shield, Key, Search, Calendar, Award, RefreshCw, Trash2, ArrowLeft, LogOut, Users, FileText } from 'lucide-react';
...
export default function AdminView({ onClose }) {
  ...
  const fetchData = async (pwd) => {
    ...
    try {
      ...
      if (data.warning) {
        ...
        // Nếu database Vercel chưa cấu hình, ta fallback lấy dữ liệu giả lập từ localStorage
        loadMockData();
      } else {
      ...
      // Chạy local không có Vercel CLI thì backend /api sẽ không chạy được -> Fallback sang mock
  ...
  const handleResetData = async () => {
    if (!window.confirm('CẢNH BÁO: Hành động này sẽ xóa toàn bộ lịch sử điểm của học sinh. Bạn có chắc chắn muốn tiếp tục?')) {
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Đã xóa sạch bảng điểm thành công.');
      } else {
        const data = await res.json();
      ...
      setSubmissions([]);
      localStorage.removeItem('admin_mock_submissions');
      alert('Đã xóa sạch bảng điểm giả lập cục bộ.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const uniqueStudents = new Set(submissions.map(s => s.phone)).size;
  const averageScore = totalSubmissions > 0 
    ? Math.round((submissions.reduce((acc, s) => acc + (s.correctCount / s.totalQuestions), 0) / totalSubmissions) * 100)
    : 0;

  // Lấy danh sách các bài học có trong bảng kết quả để lọc
  const uniqueLessons = Array.from(new Set(submissions.map(s => s.lessonTitle)));
          ...
          <form onSubmit={handleLogin} className="admin-login-form">
            ...
            <div className="admin-login-actions">
              <button type="button" className="admin-back-btn" onClick={onClose} disabled={loading}>
                <ArrowLeft size={16} /> Quay lại trang học tập
              </button>
  ...
  return (
    <div className="admin-dashboard-container animate-fade-in">
      ...
      <div className="admin-dash-header">
        ...
        <div className="admin-header-actions">
          <button className="admin-back-btn" onClick={onClose}>
            <ArrowLeft size={16} /> Trang học sinh
          </button>
      ...
      {warning && (
        <div className="warning-banner" style={{ marginBottom: '1.5rem', maxWidth: '100%' }}>
          <AlertCircleIcon />
          <span>{warning}</span>
        </div>
      )}

      {/* Thống kê Tổng quan (Stats Widgets) */}
      <div className="admin-stats-grid">
        <div className="stats-card">
          ...
          <div className="stats-info">
            <span className="stats-label">Lượt nộp bài</span>
            <span className="stats-value">{totalSubmissions}</span>
          </div>
        </div>
        ...
        <div className="stats-card">
          ...
          <div className="stats-info">
            <span className="stats-label">Số học sinh đã tham gia</span>
            <span className="stats-value">{uniqueStudents}</span>
          </div>
        </div>
          ...
          <tbody>
            {filteredSubmissions.length > 0 ? (
              filteredSubmissions.map((sub, idx) => {
                ...
                return (
                  <tr key={idx} className="animate-fade-in" style={{ animationDelay: `${idx * 0.03}s` }}>
              ...
              <tr>
                <td colSpan="6" className="table-empty-row">
                  ...
                  <p>Không tìm thấy kết quả làm bài nào phù hợp.</p>
                </td>
              </tr>
            )}
          </tbody>
